...
\STATE{\textbf{initialize} $\forall v \in V, \pmb{h}_0\of{v} = \pmb{a}_0\of{v} = \frac{1}{\abs{V}^2}$}
\WHILE{convergence criteria not reached}
\STATE{\textbf{update} $\forall v \in V, \pmb{h}_{t}\of{v} = \sum_{u \in N_{\text{out}}\of{v}}{\pmb{a}_{t-1}\of{u}}$}
\STATE{\textbf{update} $\forall v \in V, \pmb{a}_{t}\of{v} = \sum_{u \in N_{\text{in}}\of{v}}{\pmb{h}_{t-1}\of{u}}$}
\STATE{\textbf{normalize} $\forall v \in V, \pmb{h}_t\of{v} = \frac{\pmb{h}_t\of{v}}{\abs{\abs{\pmb{h}_t}}_2}, \pmb{a}_t\of{v} = \frac{\pmb{a}_t\of{v}}{\abs{\abs{\pmb{a}_t}}_2}$}
\ENDWHILE
...
\STATE{\textbf{compute} $\forall i,j \in \offf{1,\dots,\abs{V}}, R_{ij} = \frac{A_{ji}}{\sum_{k=1}^{\abs{V}}{A_{jk}}}$}
\STATE{\textbf{compute} $M = \of{1-p}R + p \frac{1}{\abs{V}}$}
