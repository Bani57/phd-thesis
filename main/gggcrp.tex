...
This research is still ongoing, to attempt to answer remaining questions posed by some experiment results, namely: what is the best approach to further improve the scalability of the GG-GAN graph generation model, as the current best performance drops for larger graphs, while preserving its high expressive power and exchangeability. The work for this project consisted of focusing on the implementation of one of the proposed approaches: exchangeable sequence modelling with Chinese Restaurant Processes (CRPs), assisted by overlap-agnostic community detection, to incur the distribution modelling benefits of autoregressive models without sacrificing generation novelty and diversity. 
...
\begin{definition}
    Given a set of samples $\mathcal{D}=\offf{\of{\mathbf{X}_i}}_{i=1}^{N}$, with $\mathbf{X}_i \in \mathbb{R}^F$, a \emph{generative model} $\mathcal{M}=\offf{f,g}$ models sample likelihoods as $\mathbb{P}\of{\mathbf{X} \in \mathcal{D}}=\sigma\of{f\of{\mathbf{X}}}$ with $f: \mathbb{R}^F \to \mathbb{R}$, and generates new samples from the dataset as $\hat{\mathbf{X}}=g\of{Z}$, from sampled latent noise $Z\in\mathbb{R}^D$ with $g:\mathbb{R}^D \to \mathbb{R}^F$. The optimal sampler $g$ is found most commonly by minimizing the expected \emph{negative log-likelihood loss}:
    \begin{equation}
...
\begin{itemize}
\item Even though they can approximate any distribution arbitrarily up to their constraints, they can \emph{waste a significant portion of capacity} modelling graphs that are isomorphic to each other - the isomorphism consistency property guaranteed by models like GG-GAN is a solution;
...
\subsubsection{Modelling and sampling of permutation-invariant graph meta-variables}
...
The full list of random variables and vectors that need to be modelled and sampled for GGG-CRP is:
...
With this setup in mind, GGG-CRP's generator pipeline design is given in Algorithm~\ref{algorithm:crp_generator} and a visual example is also given in Figure~\ref{fig:ggg_crp}. The main scalability benefits of autoregressive generation techniques for graphs lie in the reduction in memory complexity, as with this setup, only one CRP block of a graph is processed at a time.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/gggcrp/ggg_crp.pdf}
    \caption[Visual example of an adjacency matrix being generated with GGG-CRP.]{Visual example of a graph adjacency matrix being generated with GGG-CRP, one community at a time. We observe 3 communities (red, green, and blue), and thus 3 CRP rounds were required. Each of the communities was of size 3, but the green community had also one node overlapping with the red community and another with the blue one, thus those entries in the matrix were generated (and coloured) twice.}
    \label{fig:ggg_crp}
\end{figure}
...
In this section, we presented an approach on how to integrate permutation invariant metrics based on community detection into a Chinese Restaurant Process to perform exchangeable sequence modelling for graph generation. This approach, when integrated into the current best graph GAN model, GG-GAN, yielded a new variation of the model named GGG-CRP. Current results were shown to be comparable to the base model in the wake of strong resource reduction, demonstrating promise. 